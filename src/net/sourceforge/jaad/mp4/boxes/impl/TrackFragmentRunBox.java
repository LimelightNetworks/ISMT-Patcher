	...
	public void decode(MP4InputStream in) throws IOException {
		...
		for (int i = 0; i<sampleCount&&getLeft(in)>0; i++) {
			...
			if (sampleDurationPresent) {
				...
				sb.append(" duration " + sampleDuration[i] + "");
			}
			if (sampleSizePresent) {
				sampleSize[i] = in.readBytes(4);
				sb.append(" size " + sampleSize[i] + "");
			}
			if (sampleFlagsPresent) {
				sampleFlags[i] = in.readBytes(4);
				sb.append(" flags " + sampleFlags[i] + "");
			}
			if (sampleCompositionTimeOffsetPresent) {
				sampleCompositionTimeOffset[i] = in.readBytes(4);
				sb.append(" comp_time_offset " + sampleCompositionTimeOffset[i] + "");
			}
		}
		
		//System.out.println("TrackFragmentRunBox: " +
				//" sampleCount " + sampleCount + 
				//" dataOffsetPresent " + dataOffsetPresent + 
				//" dataOffset " + dataOffset +
				//" firstSampleFlagsPresent " + firstSampleFlagsPresent +
