...
public class SampleDependencyTypeBox extends FullBox {
	...
	public void decode(MP4InputStream in) throws IOException {
		...
		if (parent.hasChild(BoxTypes.COMPACT_SAMPLE_SIZE_BOX))
			sampleCount = ((SampleSizeBox) parent.getChild(BoxTypes.COMPACT_SAMPLE_SIZE_BOX)).getSampleCount();
		...
		if (parent.hasChild(BoxTypes.TRACK_FRAGMENT_RUN_BOX))
			sampleCount = ((TrackFragmentRunBox) parent.getChild(BoxTypes.TRACK_FRAGMENT_RUN_BOX)).getSampleCount();
		...
		if (sampleCount > 0) {
			...
			for(int i = 0; i<sampleCount; i++) {
				...
				sb.append(" sampleHasRedundancy " + sampleHasRedundancy[i]);
				sb.append(" sampleIsDependedOn " + sampleIsDependedOn[i]);
				sb.append(" sampleDependsOn " + sampleDependsOn[i]);
			} 
			
			//System.out.println("SampleDependencyTypeBox: " +
